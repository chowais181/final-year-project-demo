...
function pdfgenerator(exam) {
  ...
  pdf.text(exam.createdby, 125, 180);
  pdf.setFont("Times New Roman", "Bold");
  pdf.setFontSize(14);
  // pdf.setFontType("bold");
  ...
  pdf.setFontSize(14);
  // pdf.setFontType("normal");
  pdf.text(exam.startdate, 130, 210);
...
export default class ExamTable extends Component {
  ...
  getexams = () => {
    axios.get('http://localhost:5000/assesment/exam')
      .then(res => {
        this.setState({
          exams: res.data,
        })
                      ...
                      <table class="table">
                        ...
                        {this.state.exams.map(exam => (
                                ...
                                <td>
                                  ...
                                  <i class="material-icons" style={{ cursor: 'pointer' }}  onClick={() => {pdfgenerator(exam) }}>visibility |</i>
