...
function pdfgenerator(assignment) {
  ...
  pdf.text(assignment.createdby, 125, 180);
  pdf.setFont("Times New Roman", "Bold");
  pdf.setFontSize(14);
  // pdf.setFontType("bold");
  ...
  pdf.setFontSize(14);
  // pdf.setFontType("normal");
  pdf.text(assignment.startdate, 130, 210);
  ...
  pdf.setFontSize(14);
  // pdf.setFontType("normal");
  pdf.text(assignment.enddate, 440, 210);
...
export default class AssignmentTable extends Component {
  ...
  getassignments = () => {
    axios.get('http://localhost:5000/assesment/assignment')
      .then(res => {
        this.setState({
                      ...
                      <table class="table">
                        ...
                        {this.state.assignments.map(assignment => (
                                ...
                                <td>
                                  ...
                                  <i class="material-icons" style={{ cursor: 'pointer' }}  onClick={() => {pdfgenerator(assignment) }}>visibility |</i>
